...
class DispatchService(implicit fm: Materializer, system: ActorSystem) extends Directives {
  ...
  def dispatchActorFlow(sender: String): Flow[ReceivedMessage, ReceivedMessage, Unit] = {
    val in: Sink[ReceivedMessage, Unit] = Flow[ReceivedMessage]
      .map(message => ActorSubscriberMessage.OnNext(objectMapper.writeValueAsString(message)))
    ...
    val out: Source[ReceivedMessage, Unit] = Source.fromPublisher(kafkaConsumer).map[ReceivedMessage] {
      (kafkaMessage: KafkaMessage[String]) =>
        objectMapper.readValue[ReceivedMessage](kafkaMessage.message())
    }
